...
class AddMoviesViewController: UIViewController {
    ...
    func setupUIForExistingMovie() {
        ...
        txtMShortDescription.text = movie.shortDescription
        txtMPARatting.text = movie.mpaRating
        txtMCriticsRating.text = movie.criticsRating
        if !movie.imageData.isBlank,let data = Data(base64Encoded: movie.imageData), let image = UIImage(data: data) {
...
//        txtMShortDescription.delegate = self
    ...
    func createDictForSaveData() -> [String:Any] {
        ...
        if let movie = movie {
            if !movie.imageData.isBlank {
                if imgMovies != nil, !isImageUpdated , !movie.imageData.isBlank  {
                    dict["imageData"] = movie.imageData
    ...
    @IBAction func btnSaveAction(_ sender: UIButton) {
        if isValidDetails() {
            ...
            if let docID = movie?.docID {
                let userID = Auth.auth().currentUser?.uid ?? ""
                db.collection("Movies").document(userID).collection(userID).document(docID).updateData(dict)
            } else {
                let userID = Auth.auth().currentUser?.uid ?? ""
                db.collection("Movies").document(userID).collection(userID).addDocument(data: dict)
            }
            self.popVC()
        }
    }
    ...
    func isValidDetails() -> Bool {
        ...
        let isLocalImage = movie != nil && (!(movie?.imageData.isBlank ?? false))
        let isDefaultImage = (!(movie?.imageData.isBlank ?? false) && movie?.image == nil)
